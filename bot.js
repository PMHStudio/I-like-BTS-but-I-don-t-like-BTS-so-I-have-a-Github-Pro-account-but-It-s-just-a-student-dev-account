...
client.on("message", message => {
    ...
    if (message.content.startsWith("@notice")) {
      var url = "https://playentry.org/api/discuss/findNotice";
    ...
    if (message.content.startsWith("@pop")) {
      var url = "https://playentry.org/api/rankProject?type=best&limit=9";
      var options = {json: true};
    ...
    if (message.content.startsWith("@staff")) {
      var url = "https://playentry.org/api/rankProject?type=staff&limit=3";
      var options = {json: true};
    ...
    if (message.content.startsWith("@entstat")) {
      var search = args[0];
      var getuserid = "https://playentry.org/api/getUserByUsername/" + search;
      var options = {json: true};
  
      request(getuserid, options, (error, res, body) => {
        if (error) {
           return  console.log(error)
        };
        ...
        if (!error && res.statusCode == 200) {
          ...
          var getstat = "https://playentry.org/api/project/find?option=list&sort=updated&rows=0&type=project&user=" + userid;
          request(getstat, options, (error, res, body) => {
            if (error) {
               return console.log(error)
            };
